...
// Used to map track closed over variables.
typedef struct {
  uint8_t index;
  bool isLocal;
} Upvalue;
...
/**
 * We need to determine whether we're in a function
 * or executing in top level.
 */
typedef enum {
  TYPE_FUNCTION,
  TYPE_INITIALIZER,
...
/**
 * ObjFunction holds the chunk.
 * <TBD>
 */
typedef struct Compiler {
  /**
   * Inside the Compiler struct, we can’t reference the 
   * Compiler typedef since that declaration hasn’t finished yet. 
   * Instead, we give a name to the struct itself 
...
/**
 * This works, AFAICT, if the current count is +1
 * the actually number of bytes. (Update) I'm just
 * retarded and forgot about 0-indices. See if we can
 * clean this up by not applying -2 twice.
 * 
 * @param offset Equal to the count up to the jump op.
 */
 ...
 * it's a global.
 * 
 * Note that this resolution is function scope limited - 
 * as we're not checking the parent compiler's locals.
 *
 * @param compiler
 * @param name
 * @return int
...
static int addUpvalue(Compiler* compiler, uint8_t index,
  ...
  // Check to see if our upvalue has already been added.
...
static int resolveUpvalue(Compiler* compiler, Token* name) {
  ...
  // Resolve local variable reference.
  int local = resolveLocal(compiler->enclosing, name);
  // If it's not in the current scope then it's an upvalue.
  // Thus we add it to our upvalue array (or just return the index)
  // if it's already been defined in our upvalue array.
  if (local != -1) {
    compiler->enclosing->locals[local].isCaptured = true;
    return addUpvalue(compiler, (uint8_t)local, true);
  }

  // The magic happens in this recursive call.
  // If we a hit then when the stack unwinds it adds an upvalue
  // entry for all enclosing upvalues arrays beneath the locals
  // where the variable is defined to the function where it was 
  // referenced. This way we only add values to upvalue array if we
  // need to.
...
/**
This is actually a really clever way to define a static
hash-map. Since an enum definition will map it's values
...
static void function(FunctionType type) {
  Compiler compiler;
  // New compiler for each function and global scope.
...
static void ifStatement() {
  ...
  // We patch if->then jump after OP_JUMP
  // meaning that if the if expression is falsey we'll
  // jump to right after the OP_JUMP above. If it's
  // true we'll process the bytes generated from statement()
  // and hit the OP_JUMP above.
  patchJump(thenJump);
  emitByte(OP_POP);

  if (match(TOKEN_ELSE)) statement();
  patchJump(elseJump);
  // To conclude, the correctness of this implementation is
  // tightly coupled to the order of jumps w.r.t to the parse
  // function calls and emitByte calls.
}
